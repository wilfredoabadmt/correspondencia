...
Andamiaje **evergreen** para arrancar cualquier proyecto web o SaaS trabajando con **Gemini CLI**
bajo **Spec-Driven Development (SDD)**. No es un boilerplate de aplicación: es el *cómo se
construye con Gemini* — la metodología, las herramientas y la guía de arranque (`GEMINI-HELPER.md`) — listo para
montar encima el stack y el nicho que quieras.
...
| La integración de Spec Kit para Gemini CLI | Código de aplicación de ningún nicho |
| La guía `GEMINI-HELPER.md` para validación y arranque | Una app desplegable tal cual |
| La plantilla de constitución y arquitectura base | Secretos ni credenciales reales |

## Los 3 pilares

1. **Metodología SDD / Spec Kit**: Especificar antes de codificar. El flujo `constitución → specify → plan → tasks → implement` se ejecuta con los comandos de Spec Kit (`/speckit.*`) dentro de Gemini CLI.
2. **Dos espacios de trabajo**:
   - **Gemini web**: Para tutoría, validación de ideas y preparación de prompts de arranque, usando `GEMINI-HELPER.md`.
   - **Gemini CLI**: Para la implementación real dentro del repositorio, donde se ejecuta el código y se generan los artefactos.
3. **Guía `GEMINI-HELPER.md`**: Un documento exhaustivo que se pega en Gemini web para convertirlo en un asistente tutor. Cubre desde la validación de la idea de negocio hasta la generación de los prompts de arranque para SDD.
