...
export class DrizzleDashboardRepository implements IDashboardRepository {
    ...
    async getKpis({ organizationId }: { organizationId: string }): Promise<DashboardKpis> {
        ...
        const [documentsTodayResult, pendingDocumentsResult, totalDocumentsResult, overdueDocumentsResult] = await Promise.all([
            this.db
                .select({ value: count() })
            ...
            this.db
                ...
                .where(
                    and(
                        eq(schema.documents.organizationId, organizationId),
                        eq(schema.documents.status, 'Recibido'),
                        lte(schema.documents.receptionDate, fiveDaysAgo)
                    )
                ),
        ]);
        ...
        const overdueDocuments = Number(overdueDocumentsResult[0]?.value ?? 0);
